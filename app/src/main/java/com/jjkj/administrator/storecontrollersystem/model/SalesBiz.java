...
public class SalesBiz implements DaoHelper, RxHelper, BaseView {
	...
	public void updateStock(Map<String, String> map, Observer<Result> observer) {
		...
		stringMap.put("meId", map.get("person"));
	...
	public void upLoadMyPicture(Map<String, String> map, Observer<Result> observer) {
		Observable.create((ObservableOnSubscribe<Picture>) emitter -> {
		...
		}).flatMap((Function<Picture, ObservableSource<PictureUpLoadResult>>) picture ->
				mLocalService.upLoadPicture(picture))
				.flatMap((Function<PictureUpLoadResult, ObservableSource<Result>>)
						pictureUpLoadResult -> {
							map.remove("path");
							map.put("picture", String.valueOf(pictureUpLoadResult.getPicture()));
							return mLocalService.updatePerson(map);
						})
				.compose(bindOb())
				.subscribe(observer);
	}
